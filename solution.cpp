...
void AZ()
{
    ...
    double s, area_of_triangle, area_of_outer_circle, area_of_inner_circle, radious_of_outer_circle;

    while (cin >> a >> b >> c)
    {
        s = (a + b + c) / 2.0;

        area_of_inner_circle = PI * pow(sqrt(((s - a) * (s - b) * (s - c)) / s), 2);

        area_of_triangle = sqrt(s * (s - a) * (s - b) * (s - c));

        radious_of_outer_circle = (a * b * c) / (4 * area_of_triangle);

        area_of_outer_circle = PI * pow(radious_of_outer_circle, 2);

        printf("%.4lf %.4lf %.4lf\n", (area_of_outer_circle - area_of_triangle), (area_of_triangle - area_of_inner_circle), area_of_inner_circle);
    }
}
...
void AY()
{
    ...
    while (cin >> x >> y and (x != 9999.9 and y != 9999.9))
    {
        ...
        {
            if (x > v[i].first.first and x < v[i].second.first and y < v[i].first.second and y > v[i].second.second)
            {
                flag = true;
...
void AX()
{
    ...
    for (int tc = 1; tc <= T; ++tc)
    {
        ...
        printf("Case #%d: %lld\n", tc, N * myfunc(2, N - 1) % mod);
    }
}

void AW()
{
    int n, m, kmin, kmax, x, y;
